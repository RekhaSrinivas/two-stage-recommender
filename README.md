...
(LightGCN), learning-to-rank (LambdaMART), ranking evaluation, A/B
...
| **3** | LightGCN graph embeddings in PyTorch + graph ablation | ✅ done |
...
## Phase 3 results - LightGCN graph embeddings (PyTorch)
...
**Engineering note:** LightGCN first collapsed to popularity. I
...
- **A small-but-real effect can be invisible online.** The graph feature's +10% *offline*
  ...
  **~468,000 users per arm** to detect it at 80% power. Offline lift ≠ shippable online, and
  the power math tells you the traffic bill. That reconciliation is the practical takeaway.
...
- **ALS implemented directly.** The Hu-Koren-Volinsky update is implemented directly
...
- **LightGCN is "light" on purpose.** No per-layer weight matrix, no nonlinearity - He et al.
  ...
  are only the layer-0 embeddings; everything else is fixed neighbourhood averaging. Built (`src/recsys/lightgcn.py`) so the propagation is fully defensible.
